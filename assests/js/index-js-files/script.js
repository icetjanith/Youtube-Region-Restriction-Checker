...
//     tooltip.textContent = country.getAttribute('data-name');
//   });
...
document.getElementById('regionRestrictBtn').addEventListener('click', () => {
...
async function getVideoDeatils(videoid) {
  let response = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails,snippet&id=${videoid}&key=AIzaSyAr6PGDr0oXvUuUDFwgCl3fYL0FhlZC-v4`);
  if (!response.ok) {
...
function displayVideoName(data) {
  let videoNameDiv = document.getElementById("videoName");
  let videoName = data.snippet.title;
  console.log(videoName);
  let videoNameDivSection = `
                Video : <a href="https://www.youtube.com/watch?v=${data.id}" target="_blank"
                    rel="noopener noreferrer">${videoName}</a>`
  videoNameDiv.innerHTML = videoNameDivSection;
}

let restrictedCountryArray = [];
async function countries() {
  let allowedDiv = document.getElementById("allowed");
  let blockedDiv = document.getElementById("blocked");
  ...
  if (restrictedCountryArray.length == 0) {
    countryArray.forEach(country => {
      let counrtyNameDiv = document.createElement('div');
      counrtyNameDiv.className = "allowedCountry";
      counrtyNameDiv.innerHTML = country.Name;
      allowedDiv.appendChild(counrtyNameDiv);
    });
    return;
  }
  ...
  for (let i = 0; i < countryArray.length; i++) {
    ...
    for (let j = 0; j < restrictedCountryArray.length; j++) {
      console.log(countryArray[i].Code + " " + restrictedCountryArray[j]);
      if (countryArray[i].Code == restrictedCountryArray[j]) {
        isRestricted = true;
        break;
      }
    }
    let countryNameDiv = document.createElement('div');
    countryNameDiv.className = "allowedCountry";
    countryNameDiv.innerHTML = countryArray[i].Name;
    console.log(countryArray[i].Name);
    if (isRestricted) {
      blockedDiv.appendChild(countryNameDiv);
    } else {
      allowedDiv.appendChild(countryNameDiv);
    }
  }


  console.log(restrictedCountryArray);
}



function restrictCountries(countries) {
  console.log(countries);
  countries.forEach(element => {
    console.log(element);
    restrictedCountryArray.push(element);
    let counrty = document.getElementById(`${element}`);
    if (counrty) {
      counrty.style.fill = 'red';
    }
